...
def write_buckets_mgf(outfile, buckets, predict_buckets, fconfig, iontypes = ['internal{}']):
    def write_one(f, pepinfo, pred):
        ...
        ind = [i for i in range(len(peptide))]
        ...
        if 'internal{}' in fconfig.ion_types and 'internal{}' in iontypes: ions['internal{}'] = ind
...
def write_buckets(outfile, buckets, predict_buckets, iontypes = ['internal+1']):
    def write_one(f, pepinfo, pred):
        f.write('BEGIN IONS\n')
        f.write('pepinfo=' + pepinfo + '\n')
